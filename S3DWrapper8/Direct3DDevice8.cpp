...
// Diagnostic logging exposed by Direct3D8.cpp (always-on, FINAL_RELEASE-safe).
...
STDMETHODIMP CDirect3DDevice8::CreateAdditionalSwapChain( D3DPRESENT_PARAMETERS8* pPresentationParameters,IDirect3DSwapChain8** ppSwapChain)
{
	...
	if (SUCCEEDED(hr))
		...
		DEBUG_FAILED(hr);
	DiagLogProxy("Device::CreateAdditionalSwapChain BB=%ux%u Win=%d hWnd=%p hr=0x%lX\n",
		pPresentationParameters->BackBufferWidth, pPresentationParameters->BackBufferHeight,
		pPresentationParameters->Windowed, pPresentationParameters->hDeviceWindow, hr);
...
STDMETHODIMP CDirect3DDevice8::Reset( D3DPRESENT_PARAMETERS8* pPresentationParameters)
{
	...
	DiagLogProxy("Device::Reset entry: BB=%ux%u fmt=%d hWnd=%p Win=%d\n",
		pPresentationParameters->BackBufferWidth, pPresentationParameters->BackBufferHeight,
		pPresentationParameters->BackBufferFormat, pPresentationParameters->hDeviceWindow,
	...
	DiagLogProxy("Device::Reset hr=0x%lX\n", hr);
	if (FAILED(hr))
	{
		DEBUG_FAILED(hr);
	}
...
STDMETHODIMP CDirect3DDevice8::Present( CONST RECT* pSourceRect,CONST RECT* pDestRect,HWND hDestWindowOverride,CONST RGNDATA* pDirtyRegion)
{
	...
	LONG n = InterlockedIncrement(&s_presentCount);
	HRESULT hr = m_pReal->Present(pSourceRect,pDestRect,hDestWindowOverride,pDirtyRegion);
	// Log the first few frames (catch first-frame issues) and every 100th after.
	if (n <= 3 || (n % 100) == 0 || FAILED(hr))
		DiagLogProxy("Device::Present #%ld hr=0x%lX hWndOverride=%p\n", n, hr, hDestWindowOverride);
...
STDMETHODIMP CDirect3DDevice8::CreateTexture( UINT Width,UINT Height,UINT Levels,DWORD Usage,D3DFORMAT Format,D3DPOOL Pool,IDirect3DTexture8** ppTexture)
{
	...
	else
	{
		*ppTexture = NULL;
		DEBUG_FAILED(hr);
		DiagLogProxy("Device::CreateTexture FAILED %ux%u lvls=%u usage=0x%lX fmt=%d pool=%d hr=0x%lX\n",
...
STDMETHODIMP CDirect3DDevice8::CreateRenderTarget( UINT Width,UINT Height,D3DFORMAT Format,D3DMULTISAMPLE_TYPE MultiSample,BOOL Lockable,IDirect3DSurface8** ppSurface)
{
	...
	if(SUCCEEDED(hr))
		...
		DEBUG_FAILED(hr);
	DiagLogProxy("Device::CreateRenderTarget %ux%u fmt=%d MS=%d lock=%d hr=0x%lX\n",
		Width, Height, Format, MultiSample, Lockable, hr);
...
STDMETHODIMP CDirect3DDevice8::CreateDepthStencilSurface( UINT Width,UINT Height,D3DFORMAT Format,D3DMULTISAMPLE_TYPE MultiSample,IDirect3DSurface8** ppSurface)
{
	...
	if(SUCCEEDED(hr))
		...
		DEBUG_FAILED(hr);
	DiagLogProxy("Device::CreateDepthStencilSurface %ux%u fmt=%d MS=%d hr=0x%lX\n",
		Width, Height, Format, MultiSample, hr);
...
STDMETHODIMP CDirect3DDevice8::TestCooperativeLevel()
{
	...
	// Log on state changes only — games poll this every frame.
	static HRESULT s_lastTcl = S_FALSE;
	static LONG s_tclCount = 0;
	LONG n = InterlockedIncrement(&s_tclCount);
	if (hr != s_lastTcl)
	{
		DiagLogProxy("Device::TestCooperativeLevel state change: 0x%lX -> 0x%lX (call #%ld)\n",
			s_lastTcl, hr, n);
		s_lastTcl = hr;
	}
