...
void PostprocessedOutput::Output( D3DSwapChain* pSwapChain )
{
	...
	// Force a full-backbuffer viewport. DoPresent sets viewport to BB dims
	// before calling PresentData, but DrawOSD/DrawInfo/DrawLogo can leave a
	// smaller viewport active by the time we get here. Without this, the
	// SBS fullscreen triangle gets clipped to whatever sub-region the OSD
	// was drawing into (observed: 1024x768 region in the top-left of a 4K
	// backbuffer). MinDepth=0, MaxDepth=1 so SBS shader's NDC z=0 passes.
	{
		D3D10_DDI_VIEWPORT vp;
		vp.TopLeftX = 0.0f;
		vp.TopLeftY = 0.0f;
		vp.Width    = (float)pSwapChain->m_Description.BufferDesc.Width;
		vp.Height   = (float)pSwapChain->m_Description.BufferDesc.Height;
		vp.MinDepth = 0.0f;
		vp.MaxDepth = 1.0f;
		UINT nClearViewports = Commands::GetClearCount( 1, state.m_NumViewports );
		GET_ORIG.pfnSetViewports( hDevice, 1, nClearViewports, &vp );
	}
