...
{
	// Try portable path first: Language\ next to the containing DLL
	HMODULE hSelf = NULL;
	if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
		(LPCTSTR)&GetLanguageFilesPath, &hSelf) && hSelf)
	{
		TCHAR dllPath[MAX_PATH];
		if (GetModuleFileName(hSelf, dllPath, MAX_PATH))
		{
			TCHAR* slash = _tcsrchr(dllPath, '\\');
			if (slash) *slash = 0;
			TCHAR testPath[MAX_PATH];
			_tcscpy(testPath, dllPath);
			PathAppend(testPath, L"OutputMethods\\Language");
			if (GetFileAttributes(testPath) != INVALID_FILE_ATTRIBUTES)
			{
				_tcscpy(path, testPath);
				return true;
			}
		}
	}
