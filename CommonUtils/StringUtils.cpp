...
namespace common {
	namespace utils {
		...
		std::string to_utf8(const std::wstring& wide)
		{
			...
			WideCharToMultiByte(CP_UTF8, 0, wide.c_str(), (int)wide.size(), &narrow[0], sz, nullptr, nullptr);
			return narrow;
		}
		...
		std::wstring from_utf8(const std::string& narrow)
		{
			if (narrow.empty())
				return std::wstring();
			int sz = MultiByteToWideChar(CP_UTF8, 0, narrow.c_str(), (int)narrow.size(), nullptr, 0);
			std::wstring wide(sz, L'\0');
			MultiByteToWideChar(CP_UTF8, 0, narrow.c_str(), (int)narrow.size(), &wide[0], sz);
			return wide;
		}
