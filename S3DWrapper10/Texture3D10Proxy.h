/* wiz3D - ID3D10Texture3D proxy (Option B Stage 3c.2 DX10 port) */
...
class Texture3D10Proxy : public ID3D10Texture3D
{
public:
    Texture3D10Proxy(ID3D10Texture3D* real, Device10Proxy* parent);
    virtual ~Texture3D10Proxy();
    ...
    HRESULT STDMETHODCALLTYPE Map(UINT Subresource, D3D10_MAP MapType, UINT MapFlags, D3D10_MAPPED_TEXTURE3D* pMappedTex3D) override { return m_real->Map(Subresource, MapType, MapFlags, pMappedTex3D); }
    void    STDMETHODCALLTYPE Unmap(UINT Subresource) override                                                     { m_real->Unmap(Subresource); }
    void    STDMETHODCALLTYPE GetDesc(D3D10_TEXTURE3D_DESC* pDesc) override                                        { m_real->GetDesc(pDesc); }

    ID3D10Texture3D*  GetReal()   const { return m_real;   }
    Device10Proxy*    GetParent() const { return m_parent; }

private:
    ID3D10Texture3D* m_real;
