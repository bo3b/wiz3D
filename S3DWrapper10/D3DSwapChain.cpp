...
{
	{
		// Stereo-path diagnostic: log only the first ~3 calls so we don't spam.
		static int s_logCount = 0;
		if (s_logCount < 3)
		{
			SwapChainResources* pResDbg = GetCurrentPrimaryResource();
			DDILog("PresentData[%d]: pRes=%p, IsStereoActive=%d, IS_SHUTTER_OUTPUT=%d, m_bLeftEyeActive=%d, m_pOutputMethod=%p\n",
				s_logCount, (void*)pResDbg,
				pResDbg ? (int)pResDbg->IsStereoActive() : -1,
				(int)IS_SHUTTER_OUTPUT,
				(int)m_bLeftEyeActive,
				(void*)(m_pD3DDeviceWrapper ? m_pD3DDeviceWrapper->m_pOutputMethod : NULL));
			++s_logCount;
		}
	}
	RECT rect;
